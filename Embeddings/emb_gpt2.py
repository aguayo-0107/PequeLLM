...
class MultiHeadAttention(nn.Module):
    def __init__(self, n_embd: int, block_size: int, n_head: int, dropout: float = 0.1):
        ...
        self.heads = nn.ModuleList(
            [Head(n_embd=n_embd, block_size=block_size, head_size=head_size, dropout=dropout)
...
def configure_optimizer(model: nn.Module, cfg: TrainConfig) -> torch.optim.Optimizer:
    ...
    return torch.optim.AdamW(
        optim_groups, lr=cfg.lr_max, betas=(cfg.beta1, cfg.beta2)
    )
...
def maybe_load_checkpoint(
    ...
    if isinstance(raw, dict) and "model" in raw:
        ...
        if "optimizer" in raw and not fresh_optimizer:
...
def evaluate_loss(
    model: GPTModel, ds: BinTokenDataset, cfg: TrainConfig,
    device: str, amp: AmpSettings,
) -> float:
    model.eval()
    losses = []
    with torch.no_grad():
...
def collect_layer_grad_norms(model: nn.Module) -> Dict[str, float]:
    ...
    for name, param in model.named_parameters():
        ...
        layer_sumsq[layer_name] = layer_sumsq.get(layer_name, 0.0) + float(
            torch.sum(grad * grad).item()
        )
...
def validate_embedding_size(cfg: TrainConfig) -> Dict[str, str]:
    ...
    checks["embedding_size_comment"] = (
        ...
        "reasonable_for_single_gpu_or_cpu_batches"
    )
    return checks
...
def train(cfg: TrainConfig) -> None:
    ...
    start_iter, restored_run = maybe_load_checkpoint(
        checkpoint_path, model, optimizer, cfg.resume, device,
        fresh_optimizer=cfg.fresh_optimizer,
    )
    run_dir = restored_run if restored_run is not None else build_run_dir(
        ...
        with torch.autocast(device_type=amp.device_type, dtype=amp.dtype, enabled=amp.enabled):
            ...
            if probe_cache and cfg.contrastive_weight > 0 \
                ...
                loss_c = compute_contrastive_loss(
                    model, probe_cache, cfg.contrastive_temperature
                )
